...
import pytest
import allure
from hisdata.test_data import CandlestickTestData, ExpectedDataStructure
...
class TestCandlestickPositive:
    ...
    def test_get_eth_5m_with_time_range(self, api_client, validator, test_logger, save_response):
        """
        测试用例：TC_002
    ...
    @pytest.mark.positive
    @allure.title("测试获取 CRO_USDT 1天 K线数据")
    @allure.description("验证能够获取 CRO_USDT 日线数据")
    @allure.severity(allure.severity_level.NORMAL)
    def test_get_cro_1d_candlestick(self, api_client, validator, test_logger, save_response):

        test_case = CandlestickTestData.POSITIVE_TEST_CASES[2]
        params = test_case["params"]
        ...
        # 发送请求
        response_data = api_client.get_candlestick(params)
        save_response(response_data, "cro_1d")
        ...
        test_logger.info(f"✅ Test passed: Retrieved {len(data)} daily candlesticks")
